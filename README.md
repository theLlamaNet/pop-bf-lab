...
Mesh Editor distinguishes static geometry from characters and supports GLB and OBJ replacements. `Mantain original skeleton and weights` is enabled by default and keeps the previous character import behavior: the selected BF character's bind matrices are retained and its weights are transferred to the new topology. With that option disabled, a skinned GLB supplies its own bind matrices, flags and vertex weights. `Adapt bones and weights` assigns the GLB's weights to matching target BF bone slots; when slot IDs differ, it pairs remaining bones in skin order. It uses the target's bone names, bind matrices and skin flags. Surplus imported bones are ignored, and target bones without a source counterpart remain without imported weights. Weights are renormalized after dropping surplus bones; a vertex left unweighted receives the target's existing paint by proximity. Retail GEO and cooked rendering buffers are rebuilt together. The BF object's GAO gizmo links and animation hierarchy are retained. Without adaptation, different bone slots are rejected because this workflow does not create new GAO bone objects or animation resources.
...
When `Import and replace materials` is enabled, GLB base-colour textures and OBJ `map_Kd` images replace textures in the selected mesh's existing material slots. OBJ names exported as `mat_N` retain native BF slot `N`, including Blender suffixes such as `mat_N.001`; assignment no longer depends on object or `usemtl` encounter order. Before GEO serialization, non-contiguous runs created by Separate by Loose Parts or duplication are regrouped by native slot and faces plus UV-face indices are reordered together. This also keeps the original materials correct when material replacement is disabled. The importer never extends the native multi-material pack, never creates leaf-material records and never increases the geometry element count; arbitrary new material names are mapped into the existing native capacity. Imported colour factors are baked into the new texture images. Fully opaque imported materials discard incidental PNG alpha and inherited alpha-blend flags; a source factor below one retains transparency. When the option is disabled, or the mesh has no usable attached textures, the existing BF materials are retained.
