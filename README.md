...
Static mesh replacement spatially transfers original baked vertex lighting to new vertices, including per-instance GAO RLI tables and rebuilt rendering buffers. OBJ-round-tripped positions are matched at Jade Toolkit's three-decimal precision, duplicated seams use the original first vertex colour, and rebuilt RLI validity bytes are normalized to `0xFE`. This transfers the existing lighting; it does not recompute shadows for a radically different shape.

When `Import and replace materials` is enabled, GLB base-colour textures and OBJ `map_Kd` images are applied with the replacement mesh. Every imported slot is cloned under a private `0x7A` key and appended after all existing slots in the mesh's native multi-material pack. Native keys, textures, slot indices and GAO links remain byte-identical, so other scene meshes that share the pack keep their original materials. Reusing the already loaded native pack also avoids the null material pointer caused by retargeting a Sands of Time GAO to a late-added pack. Additional slots are serialized in both primary geometry and cooked buffers. Imported colour factors are baked into the images. Fully opaque imported materials discard incidental PNG alpha and inherited alpha-blend flags; a source factor below one retains transparency. When the option is disabled, or the mesh has no usable attached textures, the existing BF materials are retained; untextured slots in mixed imports receive an appended clone of an original material.
...
The Mesh Editor's compact, scrollable Import options tree contains `Import and replace materials`, `Fit size and center to the original mesh`, and `Recalculate imported mesh collision`. Collision recalculation builds a triangle collider from the final imported geometry, welds seam vertices, rebuilds normals and adjacency, and attaches a private ColMap to each direct static GAO instance. Its bounds include the instance transform. Existing per-object ColMaps are replaced; separate room collision is not removed. Skinned character collision and indirect LOD instances are unsupported and produce an explicit error instead of saving a partial replacement. All three options are off by default.
