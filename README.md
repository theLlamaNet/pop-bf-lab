...
When `Import and replace materials` is enabled, GLB base-colour textures and OBJ `map_Kd` images replace textures in the selected mesh's existing material slots. The importer never extends the native multi-material pack, never creates leaf-material records and never increases the geometry element count; imported slots in excess of the native capacity are folded into the final existing element. Imported colour factors are baked into the new texture images. Fully opaque imported materials discard incidental PNG alpha and inherited alpha-blend flags; a source factor below one retains transparency. When the option is disabled, or the mesh has no usable attached textures, the existing BF materials are retained.
