...
When `Import and replace materials` is enabled, GLB base-colour textures and OBJ `map_Kd` images replace native material textures. Materials with the same image and colour factor share a BF slot, including duplicated Blender materials. If distinct appearances exceed the editable BF slots, a Yes/No dialog offers to add the excess. Yes extends the native multi-material pack with distinct leaf records next to its existing leaves and adds GEO elements; No maps excess faces into the available slots. The engine limit is 255 slots per pack. OBJ names exported as `mat_N` retain native BF slot `N`, including Blender suffixes such as `mat_N.001`; assignment does not depend on object or `usemtl` encounter order. Non-contiguous runs are regrouped by native slot with face and UV-face indices reordered together. Imported colour factors multiply the texture channels. PNG alpha is preserved; character textures use alpha testing so intermediate PNG alpha does not make the whole mesh translucent. An explicit colour-factor alpha below 0.98 uses blending. When material replacement is disabled, the existing BF materials are retained.
